...
class GaussianModel:
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        # colors = torch.rand_like(torch.tensor(np.asarray(pcd.colors)).float().cuda())
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        selected_pts_mask = torch.logical_and(selected_pts_mask,
                                              torch.max(torch.abs(self.get_scaling), dim=1).values > self.percent_dense*scene_extent)

        stds = torch.abs(self.get_scaling[selected_pts_mask]).repeat(N,1)
    ...
    def densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size):
        ...
        if max_screen_size:
            ...
            big_points_ws = torch.abs(self.get_scaling).max(dim=1).values > 0.1 * extent
